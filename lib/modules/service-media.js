...
OnvifServiceMedia.prototype.setVideoEncoderConfiguration = function(params, callback) {
	let promise = new Promise((resolve, reject) => {
		...
		if(!('Configuration' in params)) {
				reject(new Error('The "Configuration" property was invalid'));
				return;
		}
		...
		soap_body += json2xml(params['Configuration']);

		// <Name>VideoEncoderConfig_Channel2_SubStream1</Name>
		// <UseCount>1</UseCount>
		...
		// </RateControl>
		// <H264>
		// 	<GovLength>10</GovLength>
		// 	<H264Profile>Baseline</H264Profile>
		// </H264>
		
		// <SessionTimeout>PT1M</SessionTimeout>
		...
		// 	<Address>
		// 		<Type>IPv4</Type>
		// 		<IPv4Address>239.1.1.1</IPv4Address>
		// 	</Address>
		// 	<Port>40000</Port>
		// 	<TTL>64</TTL>
		...
		let soap = this._createRequestSoap(soap_body.replace(/\n$/, ''));
